BEGIN;


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Guinness Is The Greatest Beer Ever!',
        'This is the best beer in the world! Poured from tap at a local bar. You can always tell if the pour is right because of the cascading head effect that goes on, very beautiful. Mouth feel is perfect. Head was long lasting and always there, as you would expect. Very dark in colour, as you would expect from a Guinness. Tastes of roasted barley, with a hint of chocolate and coffee. Again, it`s a Guinness, you should expect a good beer.',
        5,
        now(),
        3,
        1);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Guinness does it AGAIN!',
        'This review is for the Guinness West Indies Porter. A real gem! A deep, smooth and full bodied Porter. Taste notes of chocolate, hops and toffee with a slightly sweet finish. Mouth feel is velvety and robust without being heavy. ABV is on the higher end without tasting overly strong. This is by far a better quality drink than draught Guinness and really should be made available on tap in your local. A magnificent stout that will hopefully be a permanent feature in the Guinness range. This is how a beer like this SHOULD taste!',
        5,
        now(),
        4,
        6);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Great-tasting beer!',
        'This is a great-tasting beer with a full and robust flavor. Highly enjoyable with a steak dinner. Great complexity, robustness, and balance of dark/roast/bready malts and light-moderate earthy hop/fruity yeast flavors; with a great roast/hop bitter/sweet balance; and no cloying, acrid, astringent flavors after the finish. Lightly increasing dryness from lingering hop/roast bitterness.',
        5,
        now(),
        1,
        2);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough stout! Give me more!',
        'I didnt get my stout for this, but it is still a good beer. 4/5 would reccomend',
        4,
        now(),
        2,
        12);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Whoops! All Down Under!',
        'Pretty good. Not too sweet. Not too salty. I think thats good for a beer, right?',
        5,
        now(),
        1,
        9);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough down under',
        'I didnt get my down for this. 4/5 would not reccomend',
        4,
        now(),
        4,
        8);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('I love Guinness! Fills me with Fortitude and Boldness!',
        'Seeing 10 ratings for some of these outdated and overrated beers fills me with Fortitude because anything by Guinness far outshines them. As for Boldness, Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ornare massa eget egestas purus viverra. Euismod nisi porta lorem mollis aliquam ut porttitor leo a.',
        5,
        now(),
        4,
        4);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Yeah! Give me da love, Guinness!',
        'Love! Love! Love! But knock a few heads together! Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ornare massa eget egestas purus viverra. Euismod nisi porta lorem mollis aliquam ut porttitor leo ab sequitur.',
        5,
        now(),
        3,
        4);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Brown for sure',
        'Beer is brown, sometimes, so thats good, right?',
        4,
        now(),
        3,
        5);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough dog',
        'I didnt get my dog for this. 4/5 would not reccomend',
        4,
        now(),
        3,
        5);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('What is Jobu?',
        'I dont know what Jobu is but this beer is pretty good',
        3,
        now(),
        5,
        6);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough jobu',
        'I didnt get my jobu for this. 4/5 would not reccomend',
        4,
        now(),
        5,
        6);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Holy Toledo Batman!',
        'See, I got this reference at least',
        4,
        now(),
        2,
        7);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough holy',
        'I didnt get my holy for this. 2/5 would not reccomend',
        1,
        now(),
        2,
        7);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Put-in-Beautiful!',
        'Great color, great taste, great beer!',
        4,
        now(),
        6,
        8);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough berry',
        'I didnt get my berry for this. 4/5 would not reccomend',
        4,
        now(),
        6,
        8);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('A very tasty beer!',
        'I haven`t had a good, refreshing beer in a while and this one just hit the spot. The initial taste was soooooo refreshing and then it kept building in a very pleasant way. A very enjoyable ride all the way through to the last drop.',
        4,
        now(),
        6,
        9);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough hipster',
        'I didnt get my hipster for this, but I still recommend it 4/5.',
        4,
        now(),
        5,
        9);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Bitter = Good',
        'Idk if bitter is necessarily good, but this beer sure is!',
        4,
        now(),
        7,
        10);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough chief',
        'I didnt get my chief on with this brewski. The main reason has to do with the taste disappearing almost immediately. I guess the beer is kinda light. 5/5 would maybe recommend it, if you`re not much of a beer connosseur.',
        5,
        now(),
        7,
        10);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Big Chief (not keef)',
        'Chief is big and this beer is good',
        4,
        now(),
        7,
        11);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough big',
        'I didnt get my big for this. 2/5 would not reccomend',
        1,
        now(),
        8,
        11);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Rally Possum is a great name',
        'Thank you Brick and Barrel, very cool!',
        5,
        now(),
        8,
        12);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough possum',
        'I didnt get my possum for this. 4/5 would not reccomend',
        4,
        now(),
        8,
        12);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Beer! Holy sobriety challenge!',
        'A potent brew. I can barely pronounce the long name for this beer sober (much less drunk!), but it is exceptionally good. I highly recommend it.',
        5,
        now(),
        9,
        89);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough pinot',
        'I didnt get my pinot for this. 4/5 would not reccomend',
        4,
        now(),
        9,
        13);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('C-town sucks but this beer doesnt',
        'Woo! Im drunk!',
        4,
        now(),
        9,
        14);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough c',
        'I didnt get my c for this. 4/5 would not reccomend',
        4,
        now(),
        10,
        14);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Hope this beer is always around',
        'Hope flows freely and this beer should too',
        5,
        now(),
        10,
        15);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough flows',
        'I didnt get my flows for this. 4/5 would not reccomend',
        4,
        now(),
        10,
        15);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Before 10? Yeah right!',
        'This beer keeps me out all night!',
        4,
        now(),
        1,
        16);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough 10',
        'I didnt get my 10 for this. 4/5 would not reccomend',
        4,
        now(),
        1,
        16);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Guinness loves philosophers...',
        'Or is it the other way around? Or is it that anything by Guinness turns the drinker into a philosopher of sorts? In any case, this robust Extra Stout is nothing to take for granted. Despite living in a bottle for some undefined period of time, this stout endures in quality and preserves its taste. My caveat to those who believe that a good stout should be warm, I can`t avoid laughing at your foolishness. Stouts were made to be drunk at an ambient temperature at a low of 45 degrees and not to exceed 72 degrees. So all you self-proclaimed purists out there not full of quality but you are full of pure...something. Drink responsibly!',
        5,
        now(),
        1,
        3);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not bad! Taste is everything in a good quality beer.',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ornare massa eget egestas purus viverra. Euismod nisi porta lorem mollis aliquam ut porttitor leo a.',
        4,
        now(),
        2,
        17);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Greatest Beer Ever!',
        'This is the best beer in the world! Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ornare massa eget egestas purus viverra. Euismod nisi porta lorem mollis aliquam ut porttitor leo a.',
        5,
        now(),
        2,
        18);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough Rani!',
        'I didnt get my Rani from this beer, but at a 4/5 I will still reccommend the suds. Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ornare massa eget egestas purus viverra. Euismod nisi porta lorem mollis aliquam ut porttitor leo a.',
        4,
        now(),
        2,
        18);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Greatest Beer Ever',
        'This is the best beer in the world!',
        5,
        now(),
        3,
        19);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough berg-a-horn',
        'I didnt get my berg-a-horn for this. 4/5 would not reccomend',
        4,
        now(),
        3,
        19);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Greatest Beer Ever',
        'This is the best beer in the world!',
        5,
        now(),
        3,
        20);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough duck',
        'I didnt get my duck for this. 4/5 would not reccomend',
        4,
        now(),
        4,
        20);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Greatest Beer Ever',
        'This is the best beer in the world!',
        5,
        now(),
        4,
        21);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough rish',
        'I didnt get my rish for this. 4/5 would not reccomend',
        4,
        now(),
        4,
        21);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Can you get my favorite beer for me?',
        'Oh, caaaaaaaan doo!',
        5,
        now(),
        5,
        22);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough meseeks',
        'I didnt get my meseeks for this. 4/5 would not reccomend',
        4,
        now(),
        5,
        22);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Greatest Beer Ever',
        'This is the best beer in the world!',
        5,
        now(),
        5,
        23);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough felicia',
        'I didnt get my felicia for this. 4/5 would not reccomend',
        4,
        now(),
        6,
        23);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Greatest Beer Ever',
        'This is the best beer in the world!',
        5,
        now(),
        6,
        24);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough flag',
        'I didnt get my flag for this. 4/5 would not reccomend',
        4,
        now(),
        6,
        24);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Greatest Beer Ever',
        'This is the best beer in the world!',
        5,
        now(),
        7,
        25);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough sipa',
        'I didnt get my sipa for this. 4/5 would not reccomend',
        4,
        now(),
        7,
        25);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Greatest Beer Ever',
        'This is the best beer in the world!',
        5,
        now(),
        7,
        26);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough nano',
        'I didnt get my nano for this. 4/5 would not reccomend',
        4,
        now(),
        8,
        26);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Greatest Beer Ever',
        'This is the best beer in the world!',
        5,
        now(),
        8,
        27);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough progress',
        'I didnt get my progress for this. 4/5 would not reccomend',
        4,
        now(),
        8,
        27);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Greatest Beer Ever',
        'This is the best beer in the world!',
        5,
        now(),
        9,
        28);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough haze',
        'I didnt get my haze for this. 4/5 would not reccomend',
        4,
        now(),
        9,
        28);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Greatest Beer Ever',
        'This is the best beer in the world!',
        5,
        now(),
        9,
        29);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough frosty',
        'I didnt get my frosty for this. 4/5 would not reccomend',
        4,
        now(),
        10,
        29);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('This stusff id amzzing!',
        'Iv hwd 3 ifthes an i so drnk nw!',
        5,
        now(),
        10,
        30);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough falling',
        'I didnt get my falling for this. 4/5 would not reccomend',
        4,
        now(),
        10,
        30);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('This stusff id amzzing!',
        'Iv hwd 3 ifthes an i so drnk nw! Besht xms beeer yeh!',
        4,
        now(),
        1,
        31);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough robot santa',
        'I didnt get my robot santa for this. 4/5 would not reccomend',
        4,
        now(),
        1,
        31);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('This stusff id amzzing!',
        'Iv hwd 3 ifthes an i so drnk nw!',
        4,
        now(),
        1,
        32);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough saturday',
        'I didnt get my saturday for this. 4/5 would not reccomend',
        4,
        now(),
        2,
        32);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('This stusff id amzzing!',
        'Iv hwd 3 ifthes an i so drnk nw!',
        4,
        now(),
        2,
        33);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough big hairy american',
        'I didnt get my big hairy american for this. 4/5 would not reccomend',
        4,
        now(),
        2,
        33);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('This stusff id amzzing!',
        'Iv hwd 3 ifthes an i so drnk nw!',
        4,
        now(),
        3,
        34);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough life',
        'I didnt get my life for this. 4/5 would not reccomend',
        4,
        now(),
        3,
        34);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('This stusff id amzzing!',
        'Iv hwd 3 ifthes an i so drnk nw!',
        4,
        now(),
        3,
        35);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not enough hola',
        'I didnt get my hola for this. 4/5 would not reccomend',
        4,
        now(),
        4,
        35);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Best Beer Ever',
        'I love wheat beer and this one is the best I have had',
        5,
        now(),
        4,
        43);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Great',
        'This beer is awesome!',
        5,
        now(),
        4,
        44);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Next',
        'Almost had to spit it out',
        1,
        now(),
        5,
        36);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Very interesting',
        'Definatly has Alchol in it',
        4,
        now(),
        5,
        36);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Buy This!',
        'Excellent Brew, hopefully they keep making this!',
        4,
        now(),
        5,
        37);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Average',
        'Not my speed',
        4,
        now(),
        6,
        37);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Best Beer',
        'This is my new favorite',
        5,
        now(),
        6,
        38);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('5 Stars!',
        'This beer is awesome!',
        5,
        now(),
        6,
        38);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Not sure how the FDA approved of this one',
        'My pet turtle seems to be very sick after giving him some of this.',
        1,
        now(),
        7,
        39);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Very interesting',
        'This is one of the most unique beers I have had, it tastes great too',
        4,
        now(),
        7,
        39);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Its on fire!',
        'You gotta try this beer, its awesome. The taste is just out of this world and now I`m ready to die. lol',
        4,
        now(),
        7,
        40);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Average',
        'Mediocre beer...It doesn`t really have a satisfying, lingering taste. It just dies as soon as you take a sip.',
        4,
        now(),
        8,
        40);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Good',
        'I love wheat beer and this one is the best I have had',
        5,
        now(),
        8,
        41);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('WOW',
        'This beer is awesome!',
        5,
        now(),
        8,
        41);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Pass',
        'Great can, terrible beer though',
        1,
        now(),
        9,
        42);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Very interesting',
        'This is one of the most unique beers I have had, it tastes great too',
        4,
        now(),
        9,
        42);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Buy This!',
        'Excellent Brew',
        4,
        now(),
        9,
        43);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Average',
        'Not my style',
        4,
        now(),
        10,
        43);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Beer',
        'I love wheat beer and this one is the best I have had',
        5,
        now(),
        10,
        44);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('WOW',
        'This beer is awesome!',
        5,
        now(),
        10,
        44);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Next',
        'Great can, but clowns are terrible, not as terrible as this beer though',
        1,
        now(),
        1,
        45);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Very interesting',
        'This is one of the most unique beers I have had, it tastes great too',
        4,
        now(),
        1,
        45);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Purchase this one',
        'Excellent Brew',
        4,
        now(),
        1,
        46);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Normal',
        'Not my style',
        4,
        now(),
        2,
        46);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Pretty Great',
        'I love wheat beer',
        5,
        now(),
        2,
        47);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('WOW',
        'This beer is awesome!',
        5,
        now(),
        2,
        47);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Next',
        'Great can, but clowns are terrible, not as terrible as this beer though',
        1,
        now(),
        3,
        48);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Very interesting',
        'This is one of the most unique beers I have had, it tastes great too',
        4,
        now(),
        3,
        49);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Buy This!',
        'Excellent Brew',
        4,
        now(),
        3,
        50);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Average',
        'Not my style',
        4,
        now(),
        4,
        50);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Best Beer Ever',
        'I love wheat beer and this one is the best I have had',
        5,
        now(),
        4,
        51);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('WOW',
        'This beer is awesome!',
        5,
        now(),
        4,
        51);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Next',
        'Great can, but clowns are terrible, not as terrible as this beer though',
        1,
        now(),
        5,
        52);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Very interesting',
        'This is one of the most unique beers I have had, it tastes great too',
        4,
        now(),
        5,
        52);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Buy This!',
        'Excellent Brew',
        4,
        now(),
        5,
        53);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Average',
        'Not my style',
        4,
        now(),
        6,
        53);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Best Beer Ever',
        'I love wheat beer and this one is the best I have had',
        5,
        now(),
        6,
        54);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('WOW',
        'This beer is awesome!',
        5,
        now(),
        6,
        54);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Next',
        'Great can, but clowns are terrible, not as terrible as this beer though',
        1,
        now(),
        7,
        55);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Very interesting',
        'This is one of the most unique beers I have had, it tastes great too',
        4,
        now(),
        7,
        55);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Buy This!',
        'Excellent Brew',
        4,
        now(),
        7,
        56);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Average',
        'Not my style',
        4,
        now(),
        8,
        56);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Best Beer Ever',
        'I like it a lot...',
        5,
        now(),
        8,
        57);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('WOW',
        'My wife left me because of this beer!',
        5,
        now(),
        8,
        57);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Next',
        'Great can, but clowns are terrible, not as terrible as this beer though',
        1,
        now(),
        9,
        58);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Very interesting',
        'This is one of the most unique beers I have had, it tastes great too',
        4,
        now(),
        9,
        58);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Buy This!',
        'Excellent Brew',
        4,
        now(),
        9,
        59);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Average',
        'Not my style',
        4,
        now(),
        10,
        59);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Best Beer Ever',
        'I love wheat beer and this one is the best I have had',
        5,
        now(),
        10,
        60);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('WOW',
        'This beer is awesome!',
        5,
        now(),
        10,
        60);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Next',
        'Great can, but clowns are terrible, not as terrible as this beer though',
        1,
        now(),
        1,
        61);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Very interesting',
        'This is one of the most unique beers I have had, it tastes great too',
        4,
        now(),
        1,
        61);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Buy This!',
        'Excellent Brew',
        4,
        now(),
        1,
        62);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Average',
        'Not my style',
        4,
        now(),
        2,
        62);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Top 5',
        'Top 5 top 5 top 5 -Drake',
        5,
        now(),
        2,
        63);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('WOW',
        'This beer is awesome!',
        5,
        now(),
        2,
        63);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Next',
        'Great can, but clowns are terrible, not as terrible as this beer though',
        1,
        now(),
        3,
        64);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Very interesting',
        'This is one of the most unique beers I have had, it tastes great too',
        4,
        now(),
        3,
        64);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Buy This!',
        'Excellent Brew',
        4,
        now(),
        3,
        25);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Average',
        'Not my style',
        4,
        now(),
        4,
        25);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('More my speed',
        'Not a big beer fan but I love this one',
        5,
        now(),
        4,
        35);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('HOLY GOODNESS',
        'This beer is awesome!',
        5,
        now(),
        4,
        16);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Wonderful!',
        'Overall: Excellent. Guinness never disappoints. This is what a stout should aspire to be. Hits on all the right notes. My new favorite stout, unless I want an oatmeal stout the Founders. Pair with steak, roasted and grilled meats and veg, cheddar and chocolate.',
        5,
        now(),
        5,
        3);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Very interesting',
        'This is one of the most unique beers I have had, it tastes great too',
        4,
        now(),
        5,
        17);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Buy This!',
        'Excellent Brew',
        5,
        now(),
        5,
        28);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('It was not the worst, but definatly not the best.',
        'Not my style',
        4,
        now(),
        6,
        29);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Best Beer Ever',
        'I love wheat beer and this one is the best I have had',
        5,
        now(),
        6,
        30);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('WOW',
        'This beer is awesome!',
        5,
        now(),
        6,
        11);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Disgusting',
        'Terrible, trash',
        1,
        now(),
        7,
        26);


INSERT INTO reviews (name, description, rating, create_date, user_id, beer_id)
VALUES ('Very interesting',
        'This is one of the most unique beers I have had, it tastes great too',
        4,
        now(),
        7,
        48);


COMMIT;